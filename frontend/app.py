...
if st.button("Save Profile", key="submit_profile"):
    response = requests.post("http://localhost:8000/profile", json={"name": name, "goal": goal})
...
if st.button("Log Workout", key="submit_workout"):
    ...
    response = requests.post("http://localhost:8000/workout", json=payload)
    if response.status_code == 200:
        data = response.json()
...
if st.button("Generate Plan", key="submit_generate"):
    response = requests.post("http://localhost:8000/generate", json={"user_id": g_user_id})
    if response.status_code == 200:
        data = response.json()
